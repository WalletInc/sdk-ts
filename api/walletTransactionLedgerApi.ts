...
export enum WalletTransactionLedgerApiApiKeys {
    api_key,
}

export class WalletTransactionLedgerApi {
    ...
    public setApiKey(key: WalletTransactionLedgerApiApiKeys, value: string) {
        (this.authentications as any)[WalletTransactionLedgerApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param registerType 
     */
    public async fetchAllLedgerTransactions (startDateTime: Date, endDateTime: Date, pageNum: number, pageSize: number, registerType?: ApplicableTerminals, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: FetchAllLedgerTransactions200Response;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: FetchAllLedgerTransactions200Response;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "FetchAllLedgerTransactions200Response");
